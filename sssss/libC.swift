...
func+(right:Matrix,left:Matrix)->Matrix?{
    ...
    for(var i:Int=0 ; i <= right.mat.endIndex ;i++){
...
func*(right:Matrix,left:Matrix)->Matrix?{
    ...
    var A:Array<Double>=Array<Double>();
    for(var i:Int=1;i<=right.nor*left.noc;i++){
        var sum:Double=0;
...
func*(right:Double,left:Matrix)->Matrix?{
    var A:Array<Double>=Array<Double>();
    for(var i:Int=0;i<=left.mat.endIndex;i++){
...
func integral(f:Function,a:Double?,b:Double?)->Double{
    ...
    if(b==nil){ul=100000;}
    if(a==nil){ll = -100000;}
    for(var s:Double=ll!;s<=ul!; s+=0.002){
